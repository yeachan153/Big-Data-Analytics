...
#########################################################################################################################
# Standardise test and raw data together - z scores.
testdata_unnecessary = testdata[c(1,2,3,5)]
testdata_standardise = 
  testdata %>%
  select(activity.x, m1.x:am3.y)

full_data = rbind(all_raw_data, testdata_standardise)
activities = as.character(full_data$activity.x)
...
full_data =
  ...
  cbind(activities) %>%
  as.tibble() %>%
  mutate(activities = as.character(activities)) %>%
  rename(activity.x = activities) %>%
  select(activity.x, everything())

training_data = 
  full_data %>%
  filter(!is.na(activity.x)) 

test_data =
  full_data %>%
  filter(is.na(activity.x)) %>%
  cbind(testdata_unnecessary) %>%
  as.tibble() %>%
  select(user_id, exp_id,
         epoch, activity.x, 
         sample.x, everything())
...
write.csv(training_data, path_train,
          fileEncoding = "UTF-8", row.names = F)

write.csv(test_data, path_test,
          fileEncoding = "UTF-8", row.names = F)
